...
public interface PaymentRepository extends JpaRepository<Payment, Integer> {
    ...
    @Query( value = "SELECT \n" +
            "new com.steady.leisurethatapi.calculate.dto.CalculateAmountResultDTO(SUM(A.paymentPrice), (SELECT SUM(B.paymentPrice) FROM Payment B WHERE B.order.project.id = :projectId AND B.paymentStatus = '결제완료' GROUP BY (B.order.project.id))) \n" +
            "  FROM Payment A \n" +
